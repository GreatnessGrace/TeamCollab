...
export const create = async (
  ...
  try {
    ...
    if (existing) {
      res.status(409).json({ message: "Team name already in use" });
      return;
    }
...
export const joinTeam = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const decoded = verifyInviteToken(token) as {
      email: string;
      teamId: string;
    };
    ...
    if (!invite) {
      res.status(403).json({ message: "Invalid invite token" });
      return;
    }
    console.log(decoded.email);
    const user = await User.findOne({ email: decoded.email });
    if (!user) {
      res.status(404).json({ message: "User not registered yet" });
      return;
    }
    ...
    res
      .status(200)
      .json({ message: "Joined team successfully", teamId: team._id });
  } catch (error) {
